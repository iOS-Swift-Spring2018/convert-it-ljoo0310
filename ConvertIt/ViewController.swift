...
class ViewController: UIViewController {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                         ...
                         Formula(conversionString: "inches to cm", formula: {$0 / 0.0397}),
                         Formula(conversionString: "cm to inches", formula: {$0 * 0.0397}),
    ...
    override func viewDidLoad() {
        ...
        userInput.becomeFirstResponder() // show keyboard upon startup
        signSegment.isHidden = true // sign segment doesn't show upon openning the app
    }
...
extension ViewController: UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if conversionString.lowercased().contains("celsius".lowercased()) { // checking if celsius convserion
            signSegment.isHidden = false
        } else {
            signSegment.isHidden = true
            userInput.text = userInput.text?.replacingOccurrences(of: "-", with: "") // handles '-' sign
            signSegment.selectedSegmentIndex = 0
        }
